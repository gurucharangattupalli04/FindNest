"""
Found Items CRUD API endpoints for FindNest.
...
@router.post(
    ...
    description="Publishes a new found item listing. Requires authentication.",
)
def create_found_item(
    ...
    contact_name = item_in.contact_name or current_user.full_name or "Community Finder"
    ...
    new_item = FoundItem(
        ...
        storage_location=item_in.storage_location.strip() if item_in.storage_location else None,
        latitude=item_in.latitude,
        longitude=item_in.longitude,
        date_found=item_in.date_found,
    ...
    try:
        notification_service.process_and_notify_matches_for_found_item(db, new_item)
    except Exception as notif_err:
        logger.error(
            "Failed processing smart match notifications for found item %s: %s",
            new_item.id,
            notif_err,
            exc_info=True,
        )
...
@router.get(
    "",
    response_model=PaginatedFoundItemsResponse,
    summary="List Found Items",
    description="Public list of found items with search, categorization, and pagination.",
)
def list_found_items(
    db: Session = Depends(get_db),
    search: Optional[str] = Query(None, description="Search across title, description, brand, color, location, storage"),
    ...
    status: Optional[ItemStatus] = Query(None, description="Filter by status (e.g. active, claimed)"),
    ...
    # Search keyword filtering across title, description, brand, color, location, and storage location
    if search and search.strip():
        ...
        query = query.filter(
            or_(
                ...
                FoundItem.storage_location.ilike(term),
            )
        )
    ...
    return PaginatedFoundItemsResponse(
        items=items,
        total=total,
        page=page,
        limit=limit,
...
@router.get(
    "/{item_id}",
    response_model=FoundItemResponse,
    summary="Get Single Found Item",
    description="Retrieves complete information for a specific found item.",
)
def get_found_item(
    item_id: int,
    db: Session = Depends(get_db),
    ...
    if not item:
        raise HTTPException(
            ...
            detail=f"Found item with ID {item_id} was not found.",
        )
    return item
...
@router.put(
    ...
    description="Updates a found item report. Requires authentication and ownership.",
)
...
@router.delete(
    ...
    description="Permanently deletes a found item report. Requires authentication and ownership.",
)
def delete_found_item(
    ...
    db.delete(item)
    db.commit()
    return {"message": "Found item deleted successfully.", "id": item_id}


@router.get(
    "/{item_id}/matches",
    response_model=ItemMatchesResponse,
    summary="Find Smart AI Matches for Found Item",
    description="Analyzes active lost items using the 5-factor hybrid scoring engine and returns explainable matches >= 35% confidence.",
)
def get_found_item_matches(
    ...
    return matching_service.find_matches_for_found_item(db=db, found_item=item, limit=limit)
