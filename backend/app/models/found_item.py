"""
FoundItem SQLAlchemy ORM model for FindNest.
Includes physical custody / storage location, geolocation, finder contact, user relation, and AI metadata slots.
"""
...
class FoundItem(Base):
    ...
    storage_location: Mapped[Optional[str]] = mapped_column(
        String(255), nullable=True, comment="Safe keeping facility or desk where item is held"
    )
    latitude: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    longitude: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    date_found: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)

    # Finder / Custodian Contact Details
    ...
    user: Mapped[Optional["User"]] = relationship("User", back_populates="found_items")
    ...
    def __repr__(self) -> str:
        return f"<FoundItem id={self.id} title='{self.title}' category='{self.category}' status='{self.status}'>"
